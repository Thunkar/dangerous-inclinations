...
export type Picking = { kind: 'crit'; stepId: string } | { kind: 'peek'; stepId: string } | null
...
function SeatedPlanProvider({ me, children }: { me: Player; children: ReactNode }) {
  ...
  const pendingShip = useMemo(() => ({ ...me.ship, subsystems: pendingSubsystems }), [me.ship, pendingSubsystems])
    ...
    for (const step of steps) {
      ...
      switch (step.kind) {
        ...
        case 'move': {
          ...
          if (move.kind === 'jump') {
            const jump = getJumpOptions(position).find((o) => o.destination.wellId === move.destinationWellId)
        ...
        case 'fire': {
          ...
          const recoils = weapon && getSubsystemConfig(weapon.type).weaponStats?.hasRecoil && !step.compensateRecoil
          if (recoils) {
  ...
  const { projectedHeat, massCost, projectedFuel, issues } = useMemo(() => {
    ...
    steps.forEach((step, index) => {
      ...
      switch (step.kind) {
        ...
        case 'move': {
          ...
          if (move.kind === 'burn') {
            ...
            if (!scoop || scoop.isBroken) problems.push('Fuel scoop is broken')
              ...
              problems.push(`Scooping needs ${getSubsystemConfig('scoop').minEnergy} energy on the scoop`)
            else {
        ...
        case 'fire': {
          ...
          else if (!weapon.isPowered) problems.push(`${config.name} needs ${config.minEnergy} energy to fire`)
          else heat += weapon.allocatedEnergy
          if (weapon.type === 'missiles' && (weapon.ammo ?? 0) <= 0) problems.push('No missiles left aboard')
        ...
        case 'scan': {
          ...
          if (!sensor) problems.push('No sensor array aboard')
            ...
            problems.push(`Scanning needs ${getSubsystemConfig('sensor_array').minEnergy} energy on the sensors`)
    ...
    if (engineUses > 1) problems.push('Engines act once per turn: burn/jump or recoil compensation, not both')
  ...
  }, [steps, stepStart, pendingSubsystems, engines, compressor, me.ship, targetsInRange, availableEnergy])
  ...
  const actions = useMemo<PlayerAction[]>(() => {
    ...
    for (const s of me.ship.subsystems) {
      ...
      if (diff > 0) list.push({ playerId: me.id, type: 'allocate_energy', data: { subsystemId: s.id, amount: diff } })
      if (diff < 0)
        list.push({ playerId: me.id, type: 'deallocate_energy', data: { subsystemId: s.id, amount: -diff } })
    }
    let sequence = 0
    steps.forEach((step, index) => {
      const at = stepStart[index]
      switch (step.kind) {
        case 'rotate':
          list.push({ playerId: me.id, type: 'rotate', sequence: ++sequence, data: { targetFacing: flip(at.facing) } })
          break
        case 'move':
          if (step.move.kind === 'coast') {
            list.push({ playerId: me.id, type: 'coast', sequence: ++sequence, data: { activateScoop: step.move.scoop } })
  ...
  const allocate = useCallback(
    (subsystemId: SubsystemId, delta: number) => {
      setEnergy((prev) => {
        ...
        const total = me.ship.subsystems.reduce((sum, s) => sum + (prev[s.id] ?? s.allocatedEnergy), 0)
  ...
  const removeStep = useCallback((id: string) => {
    ...
    setPicking((p) => (p && p.stepId === id ? null : p))
  }, [])
  ...
  const pickTarget = useCallback(
    (targetId: string) => {
      setSteps((prev) => {
        const editing = picking ? prev.find((s) => s.id === picking.stepId) : undefined
        const step =
          editing ??
          [...prev].reverse().find((s) => (s.kind === 'fire' || s.kind === 'scan') && s.targetId === null) ??
  ...
  const value = useMemo<PlanContextValue>(
    () => ({
      ...
      picking,
      focusWeaponId:
        focusWeaponId ?? [...steps].reverse().find((s): s is FireStep => s.kind === 'fire')?.subsystemId ?? null,
      targets,
      targetsInRange,
      allocate,
      setEnergyTo,
