import type { GravityWell as GravityWellType, TransferPoint } from '@dangerous-inclinations/engine'
...
function getTransferSectorColor(
  ...
  for (const tp of transferPoints) {
    ...
    if (tp.fromWellId === wellId && tp.fromSector === sector) {
      const planetVisual = getGravityWellVisual(tp.toWellId)
      const baseColor = planetVisual?.color || '#FFD700'
      return lightenColor(baseColor, 0.3)
    }
    // Arrival at blackhole from planet (return journey)
    if (tp.toWellId === wellId && tp.toSector === sector) {
      const planetVisual = getGravityWellVisual(tp.fromWellId)
      const baseColor = planetVisual?.color || '#FFD700'
