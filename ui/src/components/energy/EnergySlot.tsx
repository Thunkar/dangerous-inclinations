...
import type { Subsystem, SubsystemType } from '@dangerous-inclinations/engine'
import { SUBSYSTEM_CONFIGS, getSubsystemConfig } from '@dangerous-inclinations/engine'
import { getSubsystemIcon } from '../loadout/subsystemIcons'

interface EnergySlotProps {
  subsystem: Subsystem
...
function getSubsystemColor(type: SubsystemType, isPowered: boolean, isBroken: boolean): string {
  if (isBroken) return '#666666'
  if (!isPowered) return '#444444'
...
const SlotContainer = styled(Box, {
  shouldForwardProp: prop => !['isPowered', 'isBroken', 'color'].includes(prop as string),
})<{ isPowered: boolean; isBroken: boolean; color: string }>(({ isPowered, isBroken, color }) => ({
  width: 44,
  height: 44,
  borderRadius: '50%',
  border: `2px solid ${color}`,
  backgroundColor: isPowered ? color : 'transparent',
  display: 'flex',
  alignItems: 'center',
  ...
  '&:hover': {
    ...
    boxShadow: isBroken ? 'none' : `0 0 12px ${color}`,
  },
}))
...
export function EnergySlot({
  ...
  const color = getSubsystemColor(subsystem.type, isPowered, isBroken)
