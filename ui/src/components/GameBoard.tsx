...
function GameBoardContent({
  ...
  const getWellColor = (wellId: string) => {
    const wellVisual = getGravityWellVisual(wellId)
    if (!wellVisual) return '#666'
    return wellVisual.color
  }

  const getWellRadius = (wellId: string) => {
    const wellVisual = getGravityWellVisual(wellId)
    if (!wellVisual) return 20
    return wellVisual.radius
  }
    ...
    >
      ...
      >
        <g>
          ...
          {deploymentMode &&
            availableDeploymentSectors &&
            availableDeploymentSectors.length > 0 &&
            onDeploySector && (
              <DeploymentSectors
