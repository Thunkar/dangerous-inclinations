...
import type { TurnHistoryEntry, PlayerAction } from '../types/game'
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      {botDecision.threats.length > 0 && (
        <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'background.paper' }}>
          ...
          {botDecision.threats.map((threat, idx) => (
            <Typography key={idx} variant="caption" display="block" sx={{ mb: 0.5, pl: 1 }}>
              • {threat}
            </Typography>
          ))}
      ...
      {botDecision.targets.length > 0 && (
        <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'background.paper' }}>
          ...
          {botDecision.targets.map((target, idx) => (
            <Typography key={idx} variant="caption" display="block" sx={{ mb: 0.5, pl: 1 }}>
              • {target}
            </Typography>
          ))}
      ...
      {botDecision.reasoning.length > 0 && (
        <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'background.paper' }}>
          ...
          {botDecision.reasoning.map((reason, idx) => (
            <Typography key={idx} variant="caption" display="block" sx={{ mb: 0.5, pl: 1 }}>
              • {reason}
            </Typography>
          ))}
      ...
      <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'background.paper' }}>
        ...
        {botDecision.candidates.map((candidate, idx) => (
          <Box key={idx} sx={{ mb: 1 }}>
      ...
      <Paper variant="outlined" sx={{ p: 1.5, bgcolor: 'secondary.dark', borderColor: 'secondary.main' }}>
        ...
        {botDecision.selectedCandidate.actionSummary.map((action, idx) => (
          <Typography
            key={idx}
            variant="caption"
            display="block"
