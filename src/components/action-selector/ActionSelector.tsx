...
import type { PlayerAction, BurnIntensity, Facing, Player, ActionType } from '../../types/game'
...
export function ActionSelector({
  ...
  // Get weapon range visibility and setPendingFacing from context
  const { weaponRangeVisibility, toggleWeaponRange, setPendingFacing } = useGame()
  ...
  useEffect(() => {
    setPendingFacing(targetFacing)
  }, [targetFacing, setPendingFacing])

  // Update pending action whenever settings change
  useEffect(() => {
    // Build weapon firings array from selected targets
    const weaponFirings = []
    if (selectedTargets.laser) {
      weaponFirings.push({ weaponType: 'laser', targetPlayerId: selectedTargets.laser })
    }
    if (selectedTargets.railgun) {
      weaponFirings.push({ weaponType: 'railgun', targetPlayerId: selectedTargets.railgun })
    }
    if (selectedTargets.missile) {
      weaponFirings.push({ weaponType: 'missile', targetPlayerId: selectedTargets.missile })
    }
  ...
  if (selectedTargets.railgun) {
    ...
    const hasEnginesWithMass = enginesSubsystem && enginesSubsystem.allocatedEnergy >= 1 && ship.reactionMass >= 1
    if (!hasEnginesWithMass) {
      weaponWarnings.push('⚠️ Railgun: Recoil will cause uncontrolled burn! (Need 1E in engines + 1M)')
    }
  }
  if (selectedTargets.missile) {
    if (!missilesSubsystem?.isPowered || missilesSubsystem.usedThisTurn) {
      weaponValidationErrors.push('Missile is not available')
    }
  }
  ...
  if (needsRotation && !hasRotation) {
    validationErrors.push('Need powered maneuvering thrusters (not used this turn) to change orientation')
  }
  ...
  const handleTargetSelect = (weaponType: 'laser' | 'railgun' | 'missile', targetId: string) => {
  ...
  return (
    <Paper sx={{ p: 2 }}>
      <Stack spacing={2}>
        ...
        <ActionSummary validationErrors={validationErrors} warnings={weaponWarnings} onExecute={onExecuteTurn} />
      </Stack>
    </Paper>
  )
}
