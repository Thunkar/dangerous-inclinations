...
import { applyDamageWithShields, applyCriticalHit } from '../damage'
import { createTestShip, createShipWithShields } from './fixtures/ships'
    ...
    it('should not absorb damage when shields have 0 energy allocated', () => {
      ...
      const shipWithZeroShields = {
        ...
        subsystems: ship.subsystems.map(s =>
          s.type === 'shields' ? { ...s, isPowered: true, allocatedEnergy: 0 } : s
        ),
      }
  ...
  describe('Critical hits with shields', () => {
    it('should apply critical hit to specified subsystem', () => {
      ...
      const shipWithEngines = {
        ...
        subsystems: ship.subsystems.map(s =>
          s.type === 'engines' ? { ...s, isPowered: true, allocatedEnergy: 2 } : s
        ),
      }

      const { ship: critShip, effect } = applyCriticalHit(shipWithEngines, 'engines')
