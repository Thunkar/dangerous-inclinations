...
import type { GameState, Player, DeployShipAction, TurnLogEntry } from '../types/game'
 ...
 */
export function isSectorAvailable(gameState: GameState, sector: number): boolean {
...
export function deployShip(
  ...
  let nextActiveIndex = (gameState.activePlayerIndex + 1) % gameState.players.length
  // Find the next player who hasn't deployed yet
  for (let i = 0; i < gameState.players.length; i++) {
    const checkIndex = (gameState.activePlayerIndex + 1 + i) % gameState.players.length
    // Check if this player hasn't deployed (use updated array for current player)
    const checkPlayer = checkIndex === playerIndex ? updatedPlayer : updatedPlayers[checkIndex]
