...
describe("Bot AI - Action Generation", () => {
  it("should allocate energy to subsystems when available", () => {
    ...
    const allocateActions = result.actions.filter(
  ...
  it("should generate movement action (coast or burn)", () => {
    ...
    const movementActions = result.actions.filter(
  ...
  it("should not exceed total energy budget", () => {
    ...
    const totalAllocated = allocateActions.reduce(
      (sum, action) => sum + action.data.amount,
      0
    );
...
describe("Bot AI - Difficulty Levels", () => {
  ...
  it("should produce valid actions at all difficulty levels", () => {
    ...
    const easyResult = botDecideActions(
      gameState,
      "bot1",
      createBotParameters("easy")
    );
    const mediumResult = botDecideActions(
      gameState,
      "bot1",
      createBotParameters("medium")
    );
    const hardResult = botDecideActions(
      gameState,
      "bot1",
      createBotParameters("hard")
    );
...
describe("Bot AI - Edge Cases", () => {
  ...
  it("should generate escape action when critically damaged", () => {
    ...
    const wellTransferAction = result.actions.find(
