...
import type { GameState, Player, GravityWell } from "../../models/game";
import type { LobbyState, PlayerSlot, LobbyResult } from "../../models/lobby";
import { LOBBY_CONSTANTS } from "../../models/lobby";
...
export function joinLobby(
  ...
  const emptySlotIndex = lobby.playerSlots.findIndex(
...
export function leaveLobby(lobby: LobbyState, playerId: string): LobbyResult {
  ...
  if (playerId === lobby.hostPlayerId) {
    return {
      success: false,
      error: "Host cannot leave lobby. Close the lobby instead.",
    };
  }
...
export function setPlayerReady(
  ...
  if (lobby.phase !== "lobby") {
    return {
      success: false,
      error: "Cannot change ready status after game starts",
    };
  }
 ...
 */
export function canStartGame(lobby: LobbyState): {
  canStart: boolean;
  reason?: string;
} {
  const filledSlots = lobby.playerSlots.filter(
    (slot) => slot.playerId !== null
  );
