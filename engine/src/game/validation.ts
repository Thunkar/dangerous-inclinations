 ...
 */
export function validateSingleAction(
  ship: ShipState,
  action: PlayerAction,
): ValidationResult {
  switch (action.type) {
...
function validateRotateAction(
  ...
  if (rotationSubsystem.isBroken) {
    return {
      valid: false,
      reason: "Maneuvering thrusters are broken and cannot be used",
    };
  }
...
function validateAllocateAction(
  ship: ShipState,
  ...
  if (subsystem.isBroken) {
    return {
      valid: false,
      reason: `${subsystemType} is broken and cannot receive energy`,
    };
  }
...
function validateDeallocateAction(
  ship: ShipState,
  ...
  if (subsystem.allocatedEnergy === 0) {
    return {
      valid: false,
      reason: `${subsystemType} has no energy to deallocate`,
    };
  }
...
function validateFireWeaponAction(
  ...
  if (weaponSubsystem.isBroken) {
    return {
      valid: false,
      reason: `${weaponType} is broken and cannot be used`,
    };
  }
  ...
  // Get weapon config from subsystem config
  const subsystemConfig = getSubsystemConfig(weaponType);
  const weaponConfig = subsystemConfig?.weaponStats;
  if (!weaponConfig) {
    return {
      valid: false,
      reason: `Unknown weapon type or no weapon stats: ${weaponType}`,
    };
  }
  ...
  // Check energy requirement (from subsystem config, not weapon stats)
  const minEnergy = subsystemConfig.minEnergy;
  if (weaponSubsystem.allocatedEnergy < minEnergy) {
    return {
      valid: false,
      reason: `Not enough energy (need ${minEnergy}, have ${weaponSubsystem.allocatedEnergy})`,
    };
  }
